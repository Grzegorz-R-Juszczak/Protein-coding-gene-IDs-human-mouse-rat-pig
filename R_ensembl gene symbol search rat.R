...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/9e44d795-e4b3-4a66-a10a-cd15c6dea754     ####
...
Ensembl_official_symbol_2 = getBM(attributes=c('external_gene_name', 'description', 'ensembl_gene_id', 'entrezgene_id', 'rgd_id', 'gene_biotype'), filters ='external_gene_name', values = Ensembl_official_symbol, mart = ensembl) # import of additional data from Ensembl using list of genes from Ensembl_official_symbol  for data filtering
...
colnames(Search_results_4) <- c("Anchored_input_gene", "Input_gene_symbols", "Anchored_output_gene", "Ensembl_official_symbols", "Ambiguous_Unique",  "Gene_description", "Ensembl_ID", "NCBI_ID", "RGD_ID", "Gene_type") # setting column names
head(Search_results_4, n=20) # showing first n rows
