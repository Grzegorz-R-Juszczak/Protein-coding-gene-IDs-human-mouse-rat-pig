...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/def460b5-2f2b-4ab2-bc0b-a734ca869cb1     ####
...
Ensembl_official_symbol_2 = getBM(attributes=c('external_gene_name', 'description', 'ensembl_gene_id', 'entrezgene_id', 'hgnc_id', 'gene_biotype'), filters ='external_gene_name', values = Ensembl_official_symbol, mart = ensembl) # import of additional data from Ensembl using list of genes from Ensembl_official_symbol  for data filtering
...
colnames(Search_results_4) <- c("Anchored_input_gene", "Input_gene_symbols", "Anchored_output_gene", "Ensembl_official_symbols", "Ambiguous_Unique",  "Gene_description", "Ensembl_ID", "NCBI_ID", "HGNC_ID", "Gene_type") # setting column names
head(Search_results_4, n=20) # showing first n rows
