...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/b8d3ea22-00df-4645-b40b-298cbf862363     ####
...
Committee_IDs = dplyr::select (mart_export_6, c(Gene.name, Gene.stable.ID, RGD.ID)) # selecting columns with gene symbols, Ensembl IDs and RGD IDs. 
head(Committee_IDs, n=20) # showing first n rows

Committee_IDs_2  = Committee_IDs %>% separate_longer_delim(RGD.ID, delim = ', ') # Splitting RGD IDs into rows
head(Committee_IDs_2, n=20) # showing first n rows
Duplicated_Committee_IDs_A = duplicated(Committee_IDs_2$ RGD.ID, incomparables = c(NA, 'NA'), fromLast = FALSE) # Identification of duplicated RGD IDs (forward)
head(Duplicated_Committee_IDs_A, n=20) # showing first n rows
Duplicated_Committee_IDs_B = duplicated(Committee_IDs_2$ RGD.ID, incomparables = c(NA, 'NA'), fromLast = TRUE) # Identification of duplicated RGD IDs (reverse)
...
Duplicated_Committee_IDs_4 <- dplyr::arrange(Duplicated_Committee_IDs_3, RGD.ID) # data sorting
