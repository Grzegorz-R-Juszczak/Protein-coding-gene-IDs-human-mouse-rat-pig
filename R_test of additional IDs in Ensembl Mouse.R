...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/edbf75de-5f56-4c37-b855-d6e3f91160f6     ####
...
mart_export_4 <- read.delim('mart_export_2.csv', sep=',', header = TRUE) # data import
...
Committee_IDs = dplyr::select (mart_export_6, c(Gene.name, Gene.stable.ID, MGI.ID)) # selecting columns with gene symbols, Ensembl IDs and MGI IDs. 
head(Committee_IDs, n=20) # showing first n rows

Committee_IDs_2  = Committee_IDs %>% separate_longer_delim(MGI.ID, delim = ', ') # Splitting MGI IDs into rows
head(Committee_IDs_2, n=20) # showing first n rows
Duplicated_Committee_IDs_A = duplicated(Committee_IDs_2$MGI.ID, incomparables = c(NA, 'NA'), fromLast = FALSE) # Identification of duplicated MGI IDs (forward)
head(Duplicated_Committee_IDs_A, n=20) # showing first n rows
Duplicated_Committee_IDs_B = duplicated(Committee_IDs_2$MGI.ID, incomparables = c(NA, 'NA'), fromLast = TRUE) # Identification of duplicated MGI IDs (reverse)
...
Duplicated_Committee_IDs_4 <- dplyr::arrange(Duplicated_Committee_IDs_3, MGI.ID) # data sorting
