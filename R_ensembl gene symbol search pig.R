...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/ed8fb3e5-656e-43f1-9e5f-a763a4c61121     ####
...
ensembl = useEnsembl(biomart="genes", dataset="sscrofa_gene_ensembl") # selection of Ensembl biomart database and dataset
...
Ensembl_official_symbol_2 = getBM(attributes=c('external_gene_name', 'description', 'ensembl_gene_id', 'entrezgene_id', 'vgnc', 'gene_biotype'), filters ='external_gene_name', values = Ensembl_official_symbol, mart = ensembl) # import of additional data from Ensembl using list of genes from Ensembl_official_symbol  for data filtering
...
Joined_data_grouped_3  = dplyr:: left_join(Joined_data_grouped_2, Ensembl_official_symbol_3, by = c("Ensembl_official_symbol" = "external_gene_name"), keep = FALSE, na_matches = "never") # joining the input and output data
...
colnames(Search_results_4) <- c("Anchored_input_gene", "Input_gene_symbols", "Anchored_output_gene", "Ensembl_official_symbols", "Ambiguous_Unique",  "Gene_description", "Ensembl_ID", "NCBI_ID", "VGNC_ID", "Gene_type") # setting column names
head(Search_results_4, n=20) # showing first n rows
