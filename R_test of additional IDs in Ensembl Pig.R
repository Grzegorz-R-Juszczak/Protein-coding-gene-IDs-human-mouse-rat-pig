...
#### https://data.mendeley.com/datasets/454s2vw255/1/files/e079cac4-6f67-45cd-b316-43004917f70c     ####
...
Committee_IDs = dplyr::select (mart_export_6, c(Gene.name, Gene.stable.ID, VGNC.ID)) # selecting columns with gene symbols, Ensembl IDs and VGNC IDs. 
head(Committee_IDs, n=20) # showing first n rows

Committee_IDs_2  = Committee_IDs %>% separate_longer_delim(VGNC.ID, delim = ', ') # Splitting VGNC IDs into rows
head(Committee_IDs_2, n=20) # showing first n rows
Duplicated_Committee_IDs_A = duplicated(Committee_IDs_2$VGNC.ID, incomparables = c(NA, 'NA'), fromLast = FALSE) # Identification of duplicated VGNC IDs (forward)
head(Duplicated_Committee_IDs_A, n=20) # showing first n rows
Duplicated_Committee_IDs_B = duplicated(Committee_IDs_2$VGNC.ID, incomparables = c(NA, 'NA'), fromLast = TRUE) # Identification of duplicated VGNC IDs (reverse)
...
Duplicated_Committee_IDs_4 <- dplyr::arrange(Duplicated_Committee_IDs_3, VGNC.ID) # data sorting
